...
answer.addEventListener('keyup', function () {
	...
	if (answer.value.includes('.') && answer.value.includes('@')) {
		answer.classList.remove('fail');
		answer.classList.add('pass');
		popup.classList.remove('appear');
		console.log('sweet');
	} else {
		answer.classList.remove('pass');
		answer.classList.add('fail');
	}
});

button.addEventListener('click', function (e) {
	if (answer.className === "pass") {} else if (answer.className !== "pass" || answer.value === '') {
		//stop it from continuing if your email is invalid
		e.preventDefault();
